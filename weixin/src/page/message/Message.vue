<template>
  <div class="box" ref="boxHeight">
    ...
    <back ref="heightBack" style="display: none"></back>
    <ul class="message" ref="onScoll">
      <li class="clearfix" v-for="(news,index) in messageData" :key="index">
        <div class="message-icon"></div>
        <a :href="wwwUrl + news.url" class="message-info">
          <h3>{{news.title}}</h3>
          <p>{{news.objectName}}</p>
        </a>
        <div class="message-handle">
          <a v-if="news.status === 10"
             class="publishBtn inform"
          >通知中</a>
          <a v-else-if="news.status === 0"
             class="publishBtn publis"
          >将发布</a>
          <a v-else="news.status === 11"
             class="publishBtn lose"
          >失效</a>
          <a class="edlitBtn" @click="edlitBtn(news,index)">编辑</a>
        </div>
...
<script type="es6">
  ...
  export default {
    ...
    mounted(){
      this.massageData();
      let self = this;
      let clientHeight = document.body.clientHeight - self.$refs['heightBack'].$el.clientHeight;
      window.onscroll = function (event) {
        if (self.$router.currentRoute.name !== 'message')return;
    ...
    methods: {
      ...
      onConfirm (id) {
        this.$http.get( apiRouter.DEL_INFO +'?id=' + this.id).then(response => {
      ...
      massageData(){
        ...
        this.$http.get(apiRouter.LOOK_INFO+"?" + 'page=' + this.page).then(response => {
          _this.needShow = false;
          let list = [];
          if (_this.page === 1) {
...
<style lang="less" type="text/less" scoped>
  ...
  .message-icon {
    width: 100/@baseSize;
    height: 100/@baseSize;
    background: url("../../assets/trumpet.png") no-repeat left;
